...
Needs a real PMTiles file; set VECTORTILESERVER_TEST_PMTILES to its path to run:
...
_PMTILES = os.environ.get("VECTORTILESERVER_TEST_PMTILES", "")

pytestmark = pytest.mark.skipif(
    not (_PMTILES and Path(_PMTILES).expanduser().is_file()),
    reason="VECTORTILESERVER_TEST_PMTILES not set to an existing .pmtiles file",
)
